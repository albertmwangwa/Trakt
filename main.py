...
class TraktOCRApp:
    ...
    def _create_output_dirs(self):
        ...
        if output_config.get('save_results'):
            os.makedirs(output_config.get('results_dir', './output/results'), exist_ok=True)
        
        if output_config.get('save_frames'):
            os.makedirs(output_config.get('frames_dir', './output/frames'), exist_ok=True)
    ...
    def process_frame(self, frame):
        ...
        if results:
            ...
            self.logger.info(f"Frame {self.frame_count}: Detected {len(results)} text regions")
