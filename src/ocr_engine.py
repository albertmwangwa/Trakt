...
class OCREngine:
    ...
    def _initialize_easyocr(self):
        ...
        model_storage = easyocr_config.get(
            "model_storage_directory", "./models/easyocr"
        )

        self.logger.info(
            f"Initializing EasyOCR with languages: {languages}, GPU: {gpu}"
        )
        self.ocr_reader = easyocr.Reader(
            languages, gpu=gpu, model_storage_directory=model_storage
        )
    ...
    def detect_text_tesseract(self, frame: np.ndarray) -> List[Dict]:
        ...
        data = pytesseract.image_to_data(
            frame, lang=lang, config=custom_config, output_type=pytesseract.Output.DICT
        )
    ...
    def detect_text_easyocr(self, frame: np.ndarray) -> List[Dict]:
        ...
        for detection in results_raw:
            ...
            if confidence > self.confidence_threshold:
                ...
                results.append(
                    {"text": text, "confidence": confidence, "bbox": bbox_rect}
                )
    ...
    def detect_text(self, frame: np.ndarray, preprocess: bool = True) -> List[Dict]:
        ...
        try:
            if self.engine_type == "tesseract":
                ...
                results = self.detect_text_easyocr(
                    frame
